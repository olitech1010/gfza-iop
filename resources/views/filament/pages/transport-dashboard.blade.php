...
<x-filament-panels::page>
    ...
    <div class="flex flex-col xl:flex-row gap-4 mb-6 overflow-x-auto pb-2">
        <div class="flex-1 min-w-[170px] rounded-xl px-5 py-4 text-white shadow-md" style="background-color: #1a73e8;">
        ...
        <div class="flex-1 min-w-[170px] rounded-xl px-5 py-4 text-white shadow-md" style="background-color: #10b981;">
            <div class="flex items-center gap-3">
        ...
        <div class="flex-1 min-w-[170px] rounded-xl px-5 py-4 text-white shadow-md" style="background-color: #f59e0b;">
            <div class="flex items-center gap-3">
        ...
        <div class="flex-1 min-w-[170px] rounded-xl px-5 py-4 text-white shadow-md" style="background-color: {{ $stats['pendingRequisitions'] > 0 ? '#ef4444' : '#6b7280' }};">
            <div class="flex items-center gap-3">
        ...
        <div class="flex-1 min-w-[170px] rounded-xl px-5 py-4 text-white shadow-md" style="background-color: #6366f1;">
            <div class="flex items-center gap-3">
    ...
    {{-- Recent Requisitions --}}
    <h3 class="text-lg font-semibold mb-3 dark:text-white">Recent Vehicle Requisitions</h3>
    <div class="overflow-hidden rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm mb-8">
        <table class="w-full text-sm">
            <thead class="bg-gray-50 dark:bg-gray-700/50">
                <tr>
                    <th class="px-4 py-3 text-left font-medium text-gray-600 dark:text-gray-300">Ref #</th>
                    <th class="px-4 py-3 text-left font-medium text-gray-600 dark:text-gray-300">Requester</th>
                    <th class="px-4 py-3 text-left font-medium text-gray-600 dark:text-gray-300">Destination</th>
                    <th class="px-4 py-3 text-left font-medium text-gray-600 dark:text-gray-300">Date</th>
                    <th class="px-4 py-3 text-left font-medium text-gray-600 dark:text-gray-300">Vehicle</th>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                @forelse ($recentRequisitions as $req)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                        <td class="px-4 py-3 font-medium text-gray-900 dark:text-white">{{ $req->reference_number }}</td>
                        <td class="px-4 py-3 text-gray-700 dark:text-gray-300">{{ $req->requester->name ?? '—' }}</td>
                        <td class="px-4 py-3 text-gray-700 dark:text-gray-300">{{ \Illuminate\Support\Str::limit($req->destination, 30) }}</td>
                        <td class="px-4 py-3 text-gray-700 dark:text-gray-300">{{ $req->requested_date->format('d M Y') }}</td>
                        <td class="px-4 py-3 text-gray-700 dark:text-gray-300">{{ $req->vehicle->registration_number ?? '—' }}</td>
                        <td class="px-4 py-3">
                            @php
    ...
    <h3 class="text-lg font-semibold mb-3 dark:text-white">Upcoming / Overdue Maintenance</h3>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        @foreach($upcomingServices as $svc)
            <div class="rounded-xl border {{ $svc->service_date->isPast() ? 'border-red-300 dark:border-red-700' : 'border-gray-200 dark:border-gray-700' }} bg-white dark:bg-gray-800 p-4 shadow-sm">
                <div class="flex items-center justify-between mb-2">
                    ...
                    @if($svc->service_date->isPast())
                        <span class="inline-flex items-center rounded-full bg-red-100 px-2 py-0.5 text-xs font-medium text-red-800 dark:bg-red-900/30 dark:text-red-400">Overdue</span>
                    @else
                        <span class="inline-flex items-center rounded-full bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-800 dark:bg-amber-900/30 dark:text-amber-400">Upcoming</span>
                    @endif
                </div>
